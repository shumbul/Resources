...
def write_manifest(pages):
    """
    Emit functions/pages.js so the smart 404 can fuzzy-match mistyped URLs
    against the real page list. Regenerated on every build so it cannot drift.
    """
    entries = []
    for f in pages:
        title = f.replace(".html", "").replace("-", " ").replace("_", " ").title()
        try:
            s = read(os.path.join(DOCS, f))
            m = re.search(r"<title>(.*?)</title>", s, re.S)
            if m:
                title = m.group(1).split("|")[0].strip()
        except Exception:
            pass
        entries.append({"f": f, "t": title})

    out = ("/* AUTO-GENERATED by build-seo.py. Do not edit by hand. */\n"
           "export const PAGES = " +
           json.dumps(entries, ensure_ascii=False, indent=2) + ";\n")

    path = os.path.join(DOCS, "functions", "pages.js")
...
def main():
    ...
    if check:
        ...
        path = os.path.join(DOCS, "functions", "pages.js")
        before = read(path) if os.path.exists(path) else None
        write_manifest(pages)
        after = read(path)
        manifest_stale = before != after
        if before is not None and manifest_stale:
            write(path, before)   # restore, --check must not mutate
    else:
        manifest_stale = write_manifest(pages)

    if check:
        problems = []
        if changed:
            problems.append(f"SEO tags out of date ({len(changed)}): " + ", ".join(changed))
        if manifest_stale:
            problems.append("functions/pages.js is out of date")
        if problems:
            for p in problems:
                print(p)
            sys.exit(1)
        print(f"All {len(pages)} pages have current SEO tags and the manifest is in sync.")
        return
    ...
    for f in changed:
        print("   ", f)
    if manifest_stale:
        print("    functions/pages.js regenerated")
